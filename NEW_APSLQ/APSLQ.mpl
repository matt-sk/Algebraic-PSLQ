APSLQ := proc( inxx::{list(complex),vector(complex)}, P::prime, { iterations := 1000, digits := 100, gmma :=  2/sqrt(3.),  threshold := 10^( -trunc(.24*Digits) ) }, $)::list(integer);
#option trace;
local A, B, H, ii, i, j, k, n, s, t, xx, yy, vrb, m, M, miny, pos, prev_m := 1, tmp;
    ...
    for i from 2 to n do
        for j from i-1 to 1 by -1 do
            ...
            t := AlgNearest( evalf(H[i,j]/H[j,j]), P );
    ...
    for ii from 1 to iterations do
        ...
        for i from m+1 to n do
            for j from min(i-1,m+1) to 1 by -1 do
                t := AlgNearest( expand(H[i,j]/H[j,j]), P );
        ...
        miny, pos := ListTools[FindMinimalElement]( map( abs, map(abs,yy)), position );
