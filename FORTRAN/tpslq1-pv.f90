...
!   This version is for MPFUN2015, with version #1 of mpmodf.

!   David H. Bailey     11 Mar 2015
...
parameter (gam = 1.154700538d0, idb = 3, itr = 1, kq = 0, kr = 2, ks = 2, &
...
!   Produce X vector by a custom scheme.
  ...
  write (6, *) 'input alpha value ='
  call mpwrite (6, 120, 100, al)
...
!   Perform relation search.
  ...
  tm0 = second ()
  call pslq1 (idb, gam, n, rb, eps, x, iq, r)
  tm1 = second ()
...
!   David H. Bailey     21 Feb 2015

subroutine pslq1 (idb, gam, n, rb, eps, x, iq, r)

!   Arguments are as follows:
!     Name  Type    Description
!     idb   int*4   Debug flag (0-3).
!     gam   real*8  PSLQ gamma parameter, normally set to sqrt(4/3).
...
type (mp_real) b(n,n), h(n,n), s(n), r(n), x(n), y(n), bound, rn, eps, t1, t2, &
  t3, t4
...
tm0 = second ()
call initmp (idb, n, ix, dx, eps, b, h, s, x, y, izm)
tm1 = second ()
times(1) = tm1 - tm0
...
tm0 = second ()
call itermp (idb, gam, it, n, ix, dx, eps, b, h, y, izm)
tm1 = second ()
...
5   format ('Iteration',i7,3x,'Min, max of y =',f11.6,'e',i5, &
      f11.6,'e',i5)
  endif
...
subroutine initmp (idb, n, ix, dx, eps, b, h, s, x, y, izm)
...
subroutine itermp (idb, gam, it, n, ix, dx, eps, b, h, y, izm)
...
type (mp_real) eps, teps, t1, t2, t3, t4
